...
if conjugation_choice == 0:
    ...
    conjugation_instruction = f"In '{language}', generate a phrase using the given word without conjugation."
else:
    # 1, 2, 3 mean random conjugation
    conjugation_instruction = f"In '{language}', generate a phrase using the given word in a random conjugation."

# Generate a phrase using the word in the desired foreign language
phrase_completion = client.chat.completions.create(
  model="gpt-3.5-turbo",
  messages=[
    {"role": "system", "content": f"'{conjugation_instruction}'"},
    {"role": "user", "content": f"Generate a phrase using '{word}'."}
  ]
)
generated_phrase = phrase_completion.choices[0].message

# Translate the phrase into English
translation_completion = client.chat.completions.create(
  model="gpt-3.5-turbo",
  messages=[
    {"role": "system", "content": "Translate the following phrase into English."},
    {"role": "user", "content": generated_phrase}
  ]
)
translated_phrase = translation_completion.choices[0].message
...
definition_completion = client.chat.completions.create(
  model="gpt-3.5-turbo",
  messages=[
    {"role": "system", "content": f"Provide a dictionary definition of the given word in '{language}'."},
    {"role": "user", "content": f"Define '{word}'."}
  ]
)
word_definition = definition_completion.choices[0].message
